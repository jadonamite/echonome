...
- [x] `[BE]` Rework `mirror/engine.ts` to `account.placeOrder(…)`, keeping idempotency, rate limit, kill switch and `failed`-state handling
- [x] `[BE]` DB: `proxy_grant.account_address`, plus an `executorStatus()` preflight so a paused, expired, revoked, over-budget or un-allowlisted account is recorded with a reason instead of burning gas discovering it
- [x] `[FE]` `/connect` rebuilt around deploy → fund → configure — `apps/web/app/connect/page.tsx`. Shows the account address before it costs anything to deploy, and states exactly what the authorisation can and cannot do next to the button that grants it
- [x] `[FE]` `/me`: account address, balance, caps, budget remaining, expiry countdown, pause and revoke as first-class buttons — `apps/web/components/account-panel.tsx`. Reads the CHAIN, not our database: on the one screen where someone checks they can still stop us, what we believe is not good enough
- [x] `[BE]` **First real echoes, end to end, on chain — done 2026-09-09 01:32.** ec-maker fills, and follower `0xA5695daF…`'s own account `0xFf24Fc48…` places the mirrored order at 25% of the leader's size. 6/6 confirmed successful, 49 event logs, account balance 500 → 494.98 tUSDC of real collateral spent, 5.03 tUSDC tracked against the contract's own budget. `npm run demo:follower` stands the whole thing up from a fresh wallet
- [x] `[BE]` `verify:custody` proves on chain that the executor cannot move a follower's funds — 24/24 adversarial assertions, each requiring the EXPECTED named revert
